...
void PGBackendTestFixture::setup_ec_pool()
{
  ...
  pg_pool_t pool = OSDMapTestHelpers::create_ec_pool(k, m, stripe_unit * k, pool_id);
...
//   - The primary field on every MockPGBackendListener
//   - The convenience pointers (listener, backend) when new_primary is given
//
