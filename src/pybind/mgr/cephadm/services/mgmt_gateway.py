...
class MgmtGatewayService(CephadmService):
    ...
    def get_service_discovery_endpoints(self) -> List[str]:
        ...
        # A literal IPv6 address needs to be wrapped in brackets before the
        # port portion is added (see ``wrap_ipv6``).  otherwise something
        # like ``fe80::1:8765`` would be produced which is ambiguous/invalid.
        sd_endpoints = []
