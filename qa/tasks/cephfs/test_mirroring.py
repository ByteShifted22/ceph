...
class TestMirroring(CephFSTestCase):
    ...
    def test_cephfs_mirror_multithread_snapshot_starvation(self):
        ...
        self.mount_a.create_n_files("d0/myfile", 5000)
        self.mount_a.create_n_files("d1/myfile", 10)
        self.mount_a.create_n_files("d2/myfile", 10)

        log.debug('enabling mirroring')
        self.enable_mirroring(self.primary_fs_name, self.primary_fs_id)
        # disable distribute_datasync_threads options
        log.debug('disabling cephfs_mirror_distribute_datasync_threads config')
        self.config_set('client', 'cephfs_mirror_distribute_datasync_threads', 'false')
        ...
        # Even though /d1 and /d2 had only 10 files and added while d0 is already syncing, with
        ...
        self.assertGreater(d1_sync_time_stamp, d0_sync_time_stamp)
        self.assertGreater(d2_sync_time_stamp, d0_sync_time_stamp)
    ...
    def test_cephfs_mirror_multithread_snapshot_no_starvation(self):
        ...
        log.debug('enabling mirroring')
        self.enable_mirroring(self.primary_fs_name, self.primary_fs_id)
        # Enable distribute_datasync_threads for fair scheduling of sync threads
        log.debug('enabling cephfs_mirror_distribute_datasync_threads config')
        self.config_set('client', 'cephfs_mirror_distribute_datasync_threads', 'true')
        ...
        # since /d1 and /d2 had only 10 files and added while d0 is already syncing, with
        ...
        self.assertGreater(d0_sync_time_stamp, d1_sync_time_stamp)
        self.assertGreater(d0_sync_time_stamp, d2_sync_time_stamp)
