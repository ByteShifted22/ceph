    ...
    const char* func)
{
  ...
  auto indices = std::views::iota(0u, kHash_table_max_size);
  auto wrap_indices = indices | std::views::transform([&](unsigned int i) {
                        return (lock_index + i) % kHash_table_max_size;
                      });

  for (auto i : wrap_indices) {
    LockStatTraits* traits = &traits_table[i];
...
std::atomic<mono_clock::time_point> LockStat::g_start_cycles =
    mono_clock::zero();
std::atomic<mono_clock::duration> LockStat::g_threshold_cycles =
    mono_clock::duration{};
std::atomic<mono_clock::duration> LockStat::g_threshold_cycles_when =
    mono_clock::duration{};

void
LockStat::lockstat_stop()
{
  g_threshold_cycles = mono_clock::duration{};
  g_start_cycles = mono_clock::zero();
}
...
LockStatTraits::record_wait_time(
    ...
    const mono_clock::duration wait_time,
    const LockMode mode)
{
...
{
  m_wait_duration.fill(mono_clock::duration{});
  m_wait_count.fill(0);
...
int
LockStatEntry::start(const mono_clock::duration threshold)
{
  int result = 0;
  if (LockStatTraits::g_global_enable) {
    ...
    LockStat::g_start_cycles = mono_clock::now();
...
std::pair<LockStatEntry::LockStatEntryTableT, mono_clock::duration>
LockStatEntry::show()
{
  if (LockStatTraits::g_global_enable) {
    auto total_duration = mono_clock::now() - LockStat::g_start_cycles;
    return {get_lockstat_table(), total_duration};
  } else {
    return {{}, mono_clock::duration{}};
  }
}
...
LockStatEntry::dump_formatted(Formatter* f)
{
  ...
  if (LockStat::is_lockstat_enabled()) {
    ...
    std::pair<LockStatEntryTableT, mono_clock::duration> lockstat_data = show();
