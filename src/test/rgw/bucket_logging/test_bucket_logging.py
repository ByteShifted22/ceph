...
def test_bucket_logging_list(s3_client):
    ...
    try:
        ...
        s3_client.put_object(Bucket=source_bucket, Key='test-object.txt', Body=b'test content')
...
def test_bucket_logging_info_source(s3_client):
    ...
    try:
        ...
        enabled = parse_logging_config(output)
        assert enabled.get('targetBucket') == log_bucket
        assert enabled.get('targetPrefix') == f'{source_bucket}/'
...
def test_cleanup_on_logging_config_change(s3_client):
    """Test that changing logging config switches logging to the new target bucket."""
    ...
    try:
        ...
        assert 'Contents' not in response, f"Old log bucket should be empty, found: {response.get('Contents')}"

        response = s3_client.list_objects_v2(Bucket=log_bucket_2)
        assert 'Contents' in response, "Expected log objects in new log bucket after flush"
        log_object_keys = [obj['Key'] for obj in response['Contents']]
        assert flushed_obj in log_object_keys, f"Flushed object '{flushed_obj}' not in log bucket 2: {log_object_keys}"
        verify_log_object_content(s3_client, log_bucket_2, flushed_obj, source_bucket)
...
def test_multiple_sources_disable_one(s3_client):
    ...
    try:
        ...
        log_object_keys = [obj['Key'] for obj in response['Contents']]
        assert len(log_object_keys) == 1, f"Expected only 1 log object from source_bucket_2, got {len(log_object_keys)}: {log_object_keys}"
        assert flushed_obj in log_object_keys, f"Flushed object '{flushed_obj}' not in log bucket: {log_object_keys}"
        assert log_object_keys[0].startswith(f'{source_bucket_2}/'), f"Log object should be from source_bucket_2, got: {log_object_keys[0]}"
...
def test_logging_info_after_disable(s3_client):
    ...
    try:
        ...
        output, ret = admin(['bucket', 'logging', 'info', '--bucket', source_bucket])
        assert ret == 0
        assert 'targetBucket' not in output, f"Should not have logging config after disable: {output}"
